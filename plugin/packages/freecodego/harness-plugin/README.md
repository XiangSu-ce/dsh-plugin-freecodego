...
`setDefaultEngine` and `setDefaultModel` set what new sessions start on, persist through `ctx.settings` in the `freecodego-harness` namespace, and leave existing sessions pinned to their durable plan.
