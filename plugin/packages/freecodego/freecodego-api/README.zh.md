---
description: "FreeCodeGo v1 API 的脱敏原生宿主客户端：逐请求凭据、仅 HTTPS 的部署地址，以及 bootstrap、quota、health、pricing 与 payment 路由。"
...
面向现有 FreeCodeGo v1 API 的脱敏原生宿主客户端。它适配现有的 `/api/v1/freecodego/*` bootstrap、quota、health、model-pricing 与 payment 路由，而不新建第二套后端命名空间；它要求 HTTPS 部署地址，并拒绝包含已知凭据字段的 catalog 响应。
...
本客户端要求 HTTPS 部署地址，并适配现有的 `/api/v1/freecodego/*` bootstrap、quota、health、model-pricing 与 payment 路由，而不新建第二套后端命名空间。
