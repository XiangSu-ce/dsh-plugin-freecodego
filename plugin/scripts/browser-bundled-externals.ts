...
async function collectClientBundles(
  ...
  for (const [manifestPath, manifest] of manifests) {
    ...
    // A package can declare a client it does not build here. The FreeCodeGo
    // bundle assembles `dist/client.cjs` from the client the harness-ui package
    // already builds, in its own prepack, so it has no browser build to read and
    // no third-party browser input of its own; its declarations name that client
    // through `inject` instead. Skipping it keeps the walkers' question — what
    // does a published browser build resolve — while the check below still
    // reports a package whose config exists but declares no client build.
