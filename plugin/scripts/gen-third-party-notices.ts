...
/** A copy of third-party source placed inside one of this repository's own packages. */
export interface InPackageVendoredRow {
  /** Repository-relative directory holding the copy, `/`-separated and trailing-slashed. */
  directory: string
  /** `owner/repo`, read from the provenance record's repository row. */
  origin: string
  /** The upstream repository URL the provenance record names. */
  upstream: string
  /** The license the provenance record declares. */
  license: string
}

/**
 * Read one in-package provenance record.
 *
 * The record is the declaration, and only its repository and license rows are
 * machine-readable: a copy that loses either row is a hard error rather than an
 * entry that quietly stops being disclosed. The repository row must be a plain
 * `https://github.com/owner/repo` URL, because the notices name the origin from it.
 * @param directory - repository-relative directory, `/`-separated, with a trailing slash.
 * @param text - the complete `PROVENANCE.md` contents.
 * @returns the row describing that directory.
 */
export function parseInPackageVendored(directory: string, text: string): InPackageVendoredRow {
  const fields = new Map<string, string>()
  for (const line of text.split('\n')) {
    const match = /^\| (Project|Repository|License) \| (.+?) \|$/.exec(line)
    if (match?.[1] !== undefined && match[2] !== undefined) fields.set(match[1], match[2].trim())
  }
  const upstream = fields.get('Repository')
  const license = fields.get('License')
  const missing = upstream === undefined ? 'Repository' : license === undefined ? 'License' : undefined
  if (missing !== undefined) {
    throw new Error(`gen-third-party-notices: ${directory}PROVENANCE.md has no ${missing} row; its table format changed or the copy is incomplete.`)
  }
  const origin = /^https:\/\/github\.com\/([^/]+\/[^/]+?)\/?$/.exec(upstream ?? '')?.[1]
  if (origin === undefined) {
    throw new Error(`gen-third-party-notices: ${directory}PROVENANCE.md names repository ${JSON.stringify(upstream)}; only a plain https://github.com/owner/repo URL is machine-readable.`)
  }
  return { directory, origin, upstream: upstream as string, license: license as string }
}

/**
 * Collect every in-package vendored copy.
 *
 * The tree is the authority here for the same reason it is under `vendor/`: the
 * `PROVENANCE.md` marker is found by glob, so a new copy is disclosed the moment
 * it lands rather than when someone remembers to list it. Unlike `vendor/`,
 * though, these copies are compiled into the package's published artifact — where
 * no `LICENSE` file travels with them — so the notice is the only disclosure that
 * reaches a consumer.
 * @returns one row per copy, ordered by directory.
 */
function collectInPackageVendored(): InPackageVendoredRow[] {
  return globSync('packages/*/*/src/**/PROVENANCE.md', { cwd: root })
    .map(path => path.replaceAll('\\', '/'))
    .sort()
    .map((path) => {
      const directory = path.slice(0, path.length - 'PROVENANCE.md'.length)
      const row = parseInPackageVendored(directory, readFileSync(resolve(root, path), 'utf8'))
      if (!existsSync(resolve(root, `${directory}LICENSE`))) {
        throw new Error(`gen-third-party-notices: ${directory}PROVENANCE.md declares vendored source but carries no LICENSE; the copy's own notice must travel with it.`)
      }
      if (!isPermissive(row.license)) {
        throw new Error(`gen-third-party-notices: ${directory}PROVENANCE.md declares license ${JSON.stringify(row.license)}; a non-permissive license needs the identity-scoped review this file cannot infer.`)
      }
      return row
    })
}
...
## Vendored source inside packages

Third-party source copied into a package's own \`src/\` tree rather than into \`vendor/\`. It is disclosed here because the package publishes only its compiled \`lib/\`: the copy reaches a consumer as compiled output with no upstream \`LICENSE\` file travelling beside it, so this table is the disclosure that survives publication. Each directory keeps its upstream \`LICENSE\` and a \`PROVENANCE.md\` recording the files copied and their upstream hashes.

| Directory | Origin | Upstream | License |
| --- | --- | --- | --- |
${inPackageVendored.map(row => `| [\`${row.directory}\`](${row.directory}) | \`${row.origin}\` | ${row.upstream} | ${row.license} |`).join('\n')}
`
