...
async function main() {
  ...
  // A checkout that carries no hooks config is not one this installer owns.
  //
  // The published repository is a subset of this one: `package.json` is
  // published whole, so it runs this `postinstall`, but `lefthook.yml` and the
  // pairing driver the hooks invoke are not part of that subset. Skipping is
  // the only correct outcome there — the alternative is a failed
  // `pnpm install` in a checkout nobody asked hooks for, which is how the
  // public repository's first release run died: `sync:harness` installs the
  // workspace, and the install could not find this file at all.
  if (!existsSync(join(root, 'lefthook.yml'))) return
