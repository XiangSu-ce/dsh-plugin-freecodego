...
describe('worktree-local Lefthook installer', { timeout: 90_000 }, () => {
  ...
  it('skips a checkout that carries no hooks config', async () => {
    // The published repository is a subset of this one: it receives
    // `package.json`, so it runs this installer, but not `lefthook.yml`. An
    // installer that threw here would make that checkout uninstallable, and
    // `sync:harness` — the first step of the release workflow — installs it.
    const fixture = createFixture()
    const common = commonDirectory(fixture)
    rmSync(join(fixture.main, 'lefthook.yml'), { force: true })
    rmSync(join(fixture.linked, 'lefthook.yml'), { force: true })
    ...
    expect(existsSync(hooksPath(fixture, fixture.main))).toBe(false)
    expect(existsSync(join(common, 'dsh-lefthook-install.lock'))).toBe(false)
