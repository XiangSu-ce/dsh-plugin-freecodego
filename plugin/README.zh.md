...
FreeCodeGo 安装在 Harness Profile 中，不是单独运行的命令。普通终端先安装 Harness CLI 和 pnpm：
...
`freecodego@next` 会读取当前 Harness 版本，并自动选择声明相同 `freecodego.harnessBaseline` 的最新插件版本，因此某个 Harness 基线总是解析到针对它构建并测试过的插件版本。需要可复现安装时，请改为固定一个确切版本：`npm view freecodego versions` 会列出已发布的版本，且每个版本都声明了它需要的 Harness 版本。
